...
function extract(node, parentTag, parentAttribute, parentValue) {
    ...
    if (node.attributes.length > 0) {
        for (const attributeValue of node.attributes) {
            ...
            addTraining(tag, parentTag, parentAttributeValue, attribute, value);
            for (const child of node.children) extract(child, tag, attribute, value);
        }
    } else {
        const attribute = '';
        const value = '';
...
function addTraining(tag, parentTag, parentAttributeValue, attribute, value) {
    if (tag === '!doctype') return;
    let entry;
	switch (storage.predictionCase) {
        case PREDICTION_CASE.TAG:
			entry = {'parentTag':parentTag, 'parentAttributeValue':parentAttributeValue, 'tag':tag};
            break;
        case PREDICTION_CASE.ATTRIBUTE:
			entry = {'tag':tag, 'parentTag':parentTag, 'parentAttributeValue':parentAttributeValue, 'attribute':attribute};
            break;
        case PREDICTION_CASE.VALUE:
			entry = {'tag':tag, 'attribute':attribute, 'parentTag':parentTag, 'parentAttributeValue':parentAttributeValue, 'value':value};
            break;
    }
